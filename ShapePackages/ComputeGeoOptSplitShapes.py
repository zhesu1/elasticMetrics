...
def get_optCoe_shapes_split(f_f1, f_f2, Coe_x, f1, f2, Basis_vecFields, 
    ...
    def Opts_Imm(Coe_x):
        ...
        y = energy_func_para_split(f_f1, f_f2, Coe_x, f1, f2, Basis_1forms, a, b, c, d, Tstps)
        y.backward()
    ...
    if multires == True: 
        ...
        f_f2_low = reduce_resolution(f_f2,m,n)
        ...
        def Opts_0(Coe_x):
            ...
            Coe_x = Coe_x.view(Num_basis, -1)
            Coe_x.requires_grad_()
            y0 = energy_func_para_split(f_f1_low, f_f2_low, Coe_x, f1_low, f2_low, Basis_1forms_low, a, b, c, d, Tstps)
            y0.backward()
            return np.double(y0.data.numpy()), np.double(Coe_x.grad.data.numpy().flatten())
        
        def printx0(x):
            Energy.append(Opts_0(x)[0])
        
        res0 = optimize.minimize(Opts_0, Coe_x.flatten(), method='BFGS', jac=True, callback=printx0,
                            options={'gtol': 1e-02, 'disp': False, 'maxiter': Max_ite})
        coe_x_opt = torch.from_numpy(res0.x).float().view(Num_basis, -1)
    ...
    c_f = torch.zeros(4, 3, *f1.shape[-2:])
    c_f[0], c_f[3] = f_f1, f_f2 
    
    # compute the second and the last second discrete linear curve points
    lin_f = f1 + torch.einsum("imn,t->timn", [f2 - f1, torch.tensor([1., Tstps-2])]) / (Tstps - 1) # 2*3*Num_phi_Num_theta
    c_f[1] = lin_f[0] + torch.einsum("limn,l->imn", [Basis_Sph, coe_x_opt[:,0]])
    c_f[2] = lin_f[1] + torch.einsum("limn,l->imn", [Basis_Sph, coe_x_opt[:,-1]])
    ...
    return c_f[0], c_f[-1], coe_x_opt, Energy


# the energy function for the curve obtained by perturbing curve0 with Coe_x*Basis_1forms in space of surfaces
def energy_func_para_split(f_f1, f_f2, Coe_x, f1, f2, Basis_1forms, a, b, c, d, Tstps):
    ...
    curve0 = df1 + torch.einsum("ijmn,t->tijmn", [df2 - df1, Time_points]) / (Tstps - 1)  # df1 + (df2-df1)t
    
    # calculate the new path between the new endpoints using the optimal coefficient matrix
    curve = torch.zeros(curve0.size())
    curve[0], curve[-1] = f_to_df(f_f1), f_to_df(f_f2)
...
def compute_optimal_reg_1st(c_f, idty, Basis_vecFields, a, b, c, d):
    ...
    res_reg = optimize.minimize(Opts, X, method='BFGS',
                                jac=True, callback=printx00, options={'gtol': 1e-02, 'disp': False, 'maxiter': 10})  # True
