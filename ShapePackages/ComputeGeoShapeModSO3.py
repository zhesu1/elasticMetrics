...
def compute_geodesic_shape_main(f1, f2, *, MaxDegVecFS2, MaxDegHarmSurf,
    ...
    geo, EnergyAll0 = compute_geodesic_unparaModSO3(f, f2, Cmetric = Cmetric,
...
def compute_geodesic_unparaModSO3(f1, f2, *, MaxDegVecFS2, MaxDegHarmSurf,
    ...
    if method == 'split':
        ...
        lin_f = f1_new + torch.einsum("imn,t->timn", [Rf2-f1_new, Time_points])/(Tstps-1)
        ...
        # perturbe the linear path using the optimal coefficients
        geo_f = torch.zeros(Tstps, 3, *f1.shape[-2:])
        geo_f[0], geo_f[Tstps - 1] = f, lin_f[-1]
        geo_f[1: Tstps - 1] = lin_f[1: Tstps - 1] + torch.einsum("limn,lt->timn", [Basis_Sph, Coe_x])
