...
def energy_fun_combined(f, X, Coe_x, f1, f2, Tstps, Basis_vecFields, Basis_1forms, a, b, c, d):
    # f1 and f2 are the original boundary surfaces

    df1, df2 = f_to_df(f1), f_to_df(f2)
    ...
    curve[0], curve[curve0.size(0) - 1] = f_to_df(f_gamma), curve0[curve0.size(0) - 1]
...
def get_optCoe_shapes_combined(f, Coe_x, f1, f2, Basis_vecFields, Basis_1forms, a, b, c, d, Tstps, Max_ite):

    Num_basis = Basis_1forms.size(0)

    X = torch.zeros(Basis_vecFields.size(0))
    XX = torch.cat((X, Coe_x.flatten()))

    # define the function to be optimized
    def Opts0(XX):
        X = torch.from_numpy(XX[:Basis_vecFields.size(0)]).float().requires_grad_()
        Coe_x = torch.from_numpy(XX[Basis_vecFields.size(0):]).float()
        Coe_x = Coe_x.view(Num_basis, -1).requires_grad_()

        y = energy_fun_combined(f, X, Coe_x, f1, f2, Tstps, Basis_vecFields, Basis_1forms, a, b, c, d)
        y.backward()

        XX_grad = torch.cat((X.grad, Coe_x.grad.flatten()))
    ...
    Coe_opt = torch.from_numpy(res0.x[Basis_vecFields.size(0):]).float().view(Num_basis, -1)

    return X_opt, Coe_opt, Energy
